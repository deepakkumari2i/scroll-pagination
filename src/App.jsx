...
export default function UnstyledDemo() {
  ...
  useEffect(() => {
    CustomerService.getCustomersMedium().then((data) => {
      setCustomers(getCustomers(data, 0, 10));
      setData(data);
      setLoading(false);
  ...
  const getCustomers = (data, first, last) => {
    console.log("first", first, last);
    return [...(data || [])].slice(first, last).map((d, index) => {
      d.sno = index;
  ...
  const [totalRecords, setTotalRecords] = useState(data ? data.length : 0);
  ...
  const handleScroll = (event) => {
    ...
    if (scrollTop + clientHeight >= scrollHeight - 200) {
      if (currentPage * pageSize <= totalRecords) {
        ...
        setCustomers(getCustomers(data, 0, startIndex));
      }
    }
  };
